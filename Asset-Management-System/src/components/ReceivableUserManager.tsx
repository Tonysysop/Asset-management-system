...
const ReceivableUserManager: React.FC<ReceivableUserManagerProps> = ({
  ...
  const handleAddUser = () => {
    if (assignedUsers.length >= maxQuantity) {
      alert(`Cannot add more users. Maximum quantity: ${maxQuantity}`);
      return;
    }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-gray-900">
          Assigned Users ({assignedUsers.length}/{maxQuantity})
        </h3>
        {assignedUsers.length < maxQuantity && !isAddingUser && (
          <button
            type="button"
      ...
      {assignedUsers.length === 0 && !isAddingUser && (
        <div className="text-center py-6 text-gray-500">
          ...
          <p className="text-sm">
            Click "Add User" to assign users to this receivable.
          </p>
        </div>
      )}
    </div>
